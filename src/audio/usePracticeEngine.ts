...
//   3. Implement "hold to speak": track the best score during a hold and, on
//      release, persist it as an attempt and fire `onAttempt`.
//
...
export function usePracticeEngine(
  ...
  const finishHold = useCallback(() => {
    ...
    // Record the attempt only if there was a genuine vocalisation.
    const best = Math.round(bestRef.current * 100)
    if (best > 15) onAttemptRef.current(best)
  }, [])
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (provider) {
        ...
        // Smooth the accuracy so the meter glides instead of flickering.
        smoothRef.current = smoothRef.current * 0.8 + s.accuracy * 0.2
        if (holdingRef.current) {
          bestRef.current = Math.max(bestRef.current, smoothRef.current)
          // Safety: end very long holds.
  ...
  return { status, isMic, error, frame, score, level, isHolding, start, press, release }
}
